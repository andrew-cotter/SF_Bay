...
st.write(
    "#### Most recent: "
    + str(d.iloc[-1, 3].round(1))
    + "\N{DEGREE SIGN}F on "  # temp
    + str(d.iloc[-1, 5])
    + "-"  # month
    + str(d.iloc[-1, 6])
    + "-"  # day
...
with col1:
    st.metric(
        "Today's Average Temperature", str(todays_average.round(1)) + "\N{DEGREE SIGN}F"
    )
with col2:
    st.metric(
        "Yesterday's Average Temperature",
        str(yesterdays_average.round(1)) + "\N{DEGREE SIGN}F",
        day_delta,
    )
with col3:
    st.metric(
        "7 Days Ago", str(last_week_average.round(1)) + "\N{DEGREE SIGN}F", week_delta
    )
...
interval_data = pd.DataFrame(
    daily_average.loc[daily_average.year <= 2026, :].pivot_table(
...
if len(year_outliers.index) > 0:
    with st.expander(f"Outliers in {year}"):
        st.write(
            "The following data points in this year were detected as outliers and corrected to the average temperature for that day of the year:"
        )
        st.write(year_outliers.drop_duplicates().reset_index(drop=True))

st.markdown(
    "Data from [NOAA Tides & Currents](https://tidesandcurrents.noaa.gov/stationhome.html?id=9414290) and my personal Garmin Watch."
)
