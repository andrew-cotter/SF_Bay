...
def import_data():
    ...
    d = pd.read_csv(
        "https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=water_temperature&application=NOS.COOPS.TAC.PHYSOCEAN&begin_date=19930414&end_date=19940413&station=9414290&time_zone=lst_ldt&units=english&interval=h&format=csv"
    )
    for year in np.arange(start=2023, stop=2024):
        url = (
            "https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=water_temperature&application=NOS.COOPS.TAC.PHYSOCEAN&begin_date="
            + str(year)
            + "0414&end_date="
            + str(year + 1)
            + "0413&station=9414290&time_zone=lst_ldt&units=english&interval=h&format=csv"
        )
...
def outlier_detection(df, measurement_column="temp", group_column="doy", z_thresh=3.5):
    ...
    df["zscore"] = df.groupby(group_column)[measurement_column].transform(
...
def average_daily_data(data: pd.DataFrame):
    ...
    daily_average = data.groupby(
        by=["year", "month", "day", "doy", "source"], as_index=False
    ).agg(Mean=("temp", np.mean), Sd=("temp", np.std))
    ...
    da2 = daily_average.groupby(by=["doy", "month", "day"], as_index=False).agg(
        Mean=("Mean", np.mean), Sd=("Mean", np.std), N=("Mean", len)
    )
...
def garmin_data():
    ...
    query = (
        "SELECT startTimeLocal date, minTemperature*1.8+32 temp FROM defaultdb.activities\
